...
console.log(chalk.green.bold("\t \n Welcome to Kiran Ahmed\n "));

let response = await inquirer.prompt([
  {
    name: "userInput",
    type: "number",
    message: chalk.red.bold("\tPlease Enter the amount of seconds:"),
    validate: (input) => {
      if (isNaN(input)) {
...
function startTime(val: number) {
  ...
  // ibtedaaai value ko foran zahir kre
  ...
  setInterval(() => {
    currTime = new Date();
    timeDiff = differenceInSeconds(intervalTime, currTime);

    if (timeDiff <= 0) {
      console.log(chalk.green.bold("\t \nTime is up"));
      process.exit();
    }
    min = Math.floor((timeDiff % (3600 * 24)) / 3600);
